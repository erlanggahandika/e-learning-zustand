...
export default function CardTransactionpending({ color, users }) {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        ...
        const response = await api.get(ENDPOINTS.GETALL_PEMBELIANPENDING, {
  ...
  const searchTransactions = async () => {
    try {
      ...
      const response = await api.post(ENDPOINTS.SEARCH_SUBSCRIPTIONS +"/" + search, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.data && Array.isArray(response.data.data)) {
        setUserData(response.data.data);
  ...
  try {
    ...
    const response = await api.get(
      `${ENDPOINTS.SUBSCRIPTIONS_BY_DATE}?from=${dateFromInput}&to=${dateToInput}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    setUserData(response.data.data);
  } catch (err) {
          ...
          <table className='items-center w-full bg-transparent border-collapse'>
            <thead>
              <tr>
                ...
                <th className={'px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left ' + (color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-blueGray-600 text-blueGray-200 border-blueGray-500')}>Midtrans Order</th>
                <th className={'px-4 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left ' + (color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-blueGray-600 text-blueGray-200 border-blueGray-500')}>Course</th>
                <th className={'px-4 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-right ' + (color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-blueGray-600 text-blueGray-200 border-blueGray-500')}>Price</th>
                <th className={'px-4 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center ' + (color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-blueGray-600 text-blueGray-200 border-blueGray-500')}>Paid</th>
                ...
                <th className={'px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left ' + (color === 'light' ? 'bg-blueGray-50 text-blueGray-500 border-blueGray-100' : 'bg-blueGray-600 text-blueGray-200 border-blueGray-500')}>Status</th>
            ...
            <tbody>
              {paginated.map((t, idx) => (
                <tr key={t.id || startIndex + idx}>
                  ...
                  <td className='border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4'>{t.midtransOrderId || '-'}</td>
                  <td className='border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4'>{t.course?.title || '-'}</td>
                  <td className='border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-right font-semibold'>
                    {typeof t.course?.price === 'number' ? new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(t.course.price) : '-'}
                  </td>
                  <td className='border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center'>
                    {t.paid ? <span className='text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded'>Paid</span> : <span className='text-xs bg-yellow-50 text-yellow-600 px-2 py-0.5 rounded'>Pending</span>}
                  </td>
                  ...
                  <td className='border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4'>{t.createdAt ? new Date(t.createdAt).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' }) : '-'}</td>
                  <td className='border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4'>{t.status || '-'}</td>
...
CardTransactionpending.defaultProps = {
  color: "light",
  users: sampleUsers,
};

CardTransactionpending.propTypes = {
