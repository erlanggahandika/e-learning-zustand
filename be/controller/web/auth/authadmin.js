...
export const loginBase = async (req, res) => {
  ...
  try {
    ...
    //Cek apakah user ada
    const user = await prisma.SuperUser.findUnique({ where: { email } });
...
export const getProfile = async (req, res) => {
  try {
    ...
    const user = await prisma.SuperUser.findUnique({
      where: { uuid: token },
      select: { uuid: true, email: true, name: true, avatar: true, phone: true, title: true, createdAt: true },
    });
...
export const registerBase = async (req, res) => {
  ...
  try {
    ...
    const existingUser = await prisma.SuperUser.findUnique({ where: { email } });
...
export const recoveryPassword = async (req, res) => {
  ...
  from: `"SekolahCASN Security" <${process.env.EMAIL_USER}>`,
  to: email,
      ...
      <tr>
        <td style="padding:40px 0; text-align:center;">
          <a href="#" style="text-decoration:none;">
            <h1 style="color:#2563eb; font-size:24px; margin:0; font-weight:700;">SekolahCASN</h1>
          </a>
        </td>
      </tr>
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    const user = await prisma.SuperUser.findUnique({ where: { uuid: userId } });
    ...
    await createNotification({
      ...
      message: `Terdapat perubahan password`,
      type: "Peringatan Keamanan",
      email: user.email,
      uuid: user.uuid,
    });
...
export const kelolaProfile = async (req, res) => {
  ...
  if (!userId) {
   ...
   try {
    await prisma.SuperUser.update({
      where: { uuid: user.uuid },
      data: { name, phone, avatar: uploadedImage, title },
    })

    await createNotification({ 
      title: "Perubahan Profile",
      message: `Profile berhasil diubah`,
      type: "Perubahan Profile anda",
      email: user.email,
      uuid: user.uuid
    });
...
export const countUser = async (req, res) => {
  ...
  try {
    ...
    endOfLastMonth.setDate(0); // tanggal terakhir bulan lalu
    endOfLastMonth.setHours(23, 59, 59, 999);

    const countLastMonth = await prisma.user.count({
      where: {
        createdAt: {
    ...
    // Hitung arrow dan percent
    const arrow = count >= countLastMonth ? "up" : "down";
    const percent =
      countLastMonth === 0
        ? 100
        : ((count - countLastMonth) / countLastMonth) * 100;
